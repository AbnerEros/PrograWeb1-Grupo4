...
function updateSearch(search) {
    ...
    if ( search == '' ) {
        ...
        for(let i in CONTENT_MOVIES) {
            if ( !CONTENT_MOVIES[i].serie_name.normalize('NFD').replace(/[\u0300-\u036f]/g,"").toLowerCase().includes(search.normalize('NFD').replace(/[\u0300-\u036f]/g,"").toLowerCase()) )
                continue;
        ...
        for(let i in CONTENT_SERIES) {
            if ( !CONTENT_SERIES[i].serie_name.normalize('NFD').replace(/[\u0300-\u036f]/g,"").toLowerCase().includes(search) )
                continue;
